...
public class EventManager {
	...
	/**
	 * @param day   day of the event,should be in [1,365]
	...
	public static int book(int day, int start, int end) {
		...
		if (!(end<=24))
			assert false;
		//判断前置条件
		if (temp.containsKey((day-1)*24+start))
			temp.put((day-1)*24+start,temp.get((day-1)*24+start)+1);
		else
			temp.put((day-1)*24+start,1);
		//将start时间点处的所产生的事件数目加1
		if (temp.containsKey((day-1)*24+end))
			temp.put((day-1)*24+end,temp.get((day-1)*24+end)-1);
		else
			temp.put((day-1)*24+end,-1);
		//将end事件点处所产生的事件数减1
		int active = 0, ans = 0;
